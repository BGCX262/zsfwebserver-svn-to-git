...
import net.sf.cindy.filter.ConnectFlow;
...
/**
 * Consle socket channel session. Application can override buildSession method
...
public class ServerSocketChannelSession extends AbstractChannelSession {
	...
	protected ReactorHandler getReactorHandler() {
		return new ChannelReactorHandler() {
			...
			private boolean isReliance(SocketChannel sc) throws IOException {
				SocketAddress address = sc.socket().getRemoteSocketAddress();
				if (!ConnectFlow.isReliance(address)) {
					sc.close();
					return false;
				}
				return true;
	...
	protected void doStart() throws IOException {
		...
		try {
			...
			socket.bind(localAddr == null ? new InetSocketAddress(0) : localAddr, Configuration.getAcceptorBacklog());
		} catch (IOException e) {
			doClose();
			throw e;
		}
