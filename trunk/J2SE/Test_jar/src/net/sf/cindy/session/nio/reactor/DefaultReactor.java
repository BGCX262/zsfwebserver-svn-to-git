...
public class DefaultReactor implements Reactor {
    ...
    /*
     * Asynchronous ReadOp and WriteOp {runInSeparateThread}
     * Changed XiaoJun @2009-10-12
     */
    ...
    private void changeInterest(ReactorHandlerInterest interest) {
        ...
        // write completed
        if ((interest.ops & OP_WRITE) == 0) //It isn't timeout(XiaoJun Chang)
    ...
    private void checkSessionTimeout() {
        ...
        // check timeout
        for (Iterator iter = registered.values().iterator(); iter
                .hasNext();) {
            Attachment attachment = (Attachment) iter.next();
            ReactorHandler handler = attachment.handler;
            int newIdleTime = interval + attachment.idleTime;
            int sessionTimeout = handler.getSession().getSessionTimeout();
            int maxSessionTimeout = sessionTimeout * 10;
			if (sessionTimeout > 0 && attachment.writing && newIdleTime > maxSessionTimeout) {
            	attachment.idleTime = 0;
                handler.onTimeout();
            	log.info("Check session timeout when I/O operate timeout max timeout " + handler.getSession().getRemoteAddress());
            	continue;
			}
            ...
            if (newIdleTime > sessionTimeout) {
            	attachment.idleTime = 0;
            	handler.onTimeout();
            	if (attachment.writing) {
					log.info("Check writing session timeout " + handler.getSession().getRemoteAddress());
				}
            } else
                attachment.idleTime = newIdleTime;
        }
    ...
    private void processSelectedKeys() {
		for (Iterator iter = selector.selectedKeys().iterator(); iter.hasNext();) {
			...
			Attachment attachment = (Attachment) key.attachment();
			final ReactorHandler handler = attachment.handler;

			if (key.isValid() && key.isWritable()) // do not check session timeout when writing
			...
			if (key.isValid() && key.isReadable())
				dispatcher.dispatch(handler.getSession(), new Runnable() {
					public void run() {
						handler.onReadable();
					}});
