...
public class Cmd {
	...
	private static final Log LOG = LogFactory.getLog(Cmd.class);
	...
	public void appendAll(byte[] b) {
		response = DataFactory.addByteArray(response, b.clone());
	}
	
	public void appendAll(Cmd c) {
	...
	public Cmd getCmdResponse(int start, int len) {
		return Cmd.getInstance(DataFactory.get(response, start, len));
	}
	 ...
	 * @return
	 */
	public static Cmd getInstance(byte[] response) {
		Cmd cmd = new Cmd();
	 ...
	 * @return
	 */
	public static Cmd getInstance(Cmd cmd) {
		Cmd temp = new Cmd();
		temp.response = cmd.response.clone();
		return cmd;
	}
